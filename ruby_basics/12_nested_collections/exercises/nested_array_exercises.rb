def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  two_d_array = Array.new(number_of_rows) {Array.new(seats_per_row)}
  two_d_array
end
...
def delete_seat_from_row(chart, row_index, seat_index)
  ...
  # gets the index of nested array then deletes element inside that array
...
def count_empty_seats(chart)
  ...
  empty_seats = chart.flatten.count(nil)
  empty_seats
end
...
def find_favorite(array_of_hash_objects)
  ...
  # find method to return first matching  key, value pair
  array_of_hash_objects.find { |hash_object| hash_object.key?(:is_my_favorite?) && hash_object.value?(true) }
end
