...
def add_information_about_language(languages, language_name, info_key, info_value)
  ...
  # access hash similar to 1d hash and assign new key, value pair
...
def delete_information_about_language(languages, language_name, info_key)
  ...
  # returns block if no key is found
  languages[language_name].delete(info_key) { |key| puts "key #{key} not found" }
  return languages
end
...
def delete_language(languages, language_name)
  ...
  # returns block if no key is found
  languages.delete(language_name) { |key| puts "Key #{key} not found" }
  return languages 
end
